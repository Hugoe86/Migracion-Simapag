...
CREATE view View_Insert_Diverso_
 as 
SELECT  
 DISTINCT( c.rpu)
	,p.Usuario_ID
...
WHERE  isnumeric(c.rpu) = 1  and c.cveejecucion  like '%5%' and c.rpu in (SELECT r.rpu from Simapag_09_01_2016.dbo.recibos as r WHERE r.corte=1)
...
SELECT  RIGHT('0000000000' + 
	 ...
	 +(select isnull(max(No_Diverso), 0) from Simapag_20161015.dbo.Ope_Cor_Diversos) AS VARCHAR(10)), 10)
	 ,RIGHT('0000000000' + 
	 CAST(ROW_NUMBER() OVER(ORDER BY vista.rpu) 
	 +(select isnull(max(No_Diverso), 0) from Simapag_20161015.dbo.Ope_Cor_Diversos) AS VARCHAR(10)), 10)+'D',*
FROM View_Insert_Diverso_ vista


SELECT * from Ope_Cor_Diversos